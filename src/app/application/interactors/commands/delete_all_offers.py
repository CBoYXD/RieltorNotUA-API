...
class DeleteAllOffersRequest:
    pass

class DeleteAllOffersResponse(TypedDict):
    success: bool

class DeleteAllOffersInteractor(InteractorStrict[DeleteAllOffersRequest, DeleteAllOffersResponse]):
    def __init__(
        self, 
    ...
    async def __call__(self, request_data: DeleteAllOffersRequest) -> DeleteAllOffersResponse:
        user_id = None
        await self._auth_service.authorize_action()
        
        await self._offer_command_gateway.delete_all(user_id)
        await self._transaction_manager.commit()
        
        return DeleteAllOffersResponse(success=True)
